...
mod:hook_origin(assault_new, "update_function", function(parent, ui_renderer, widget, template, crosshair_settings, dt, t, draw_hit_indicator)
	...
	if yaw and pitch then
		...
		x, y = crosshairs_fix.crosshair_rotation(spread_offset_x, spread_offset_y, top_style.angle, top_style.size[1]/2, 2)

		top_style.offset[1] = x
		top_style.offset[2] = y

		local bottom_style = style.bottom
		x, y = crosshairs_fix.crosshair_rotation(spread_offset_x, spread_offset_y, bottom_style.angle, bottom_style.size[1]/2, 2)

		bottom_style.offset[1] = x
		bottom_style.offset[2] = y

		local left_style = style.left
		x, y = crosshairs_fix.crosshair_rotation(spread_offset_x, spread_offset_y, left_style.angle, left_style.size[1]/2, 2)

		left_style.offset[1] = x
		left_style.offset[2] = y

		local right_style = style.right
		x, y = crosshairs_fix.crosshair_rotation(spread_offset_x, spread_offset_y, right_style.angle, right_style.size[1]/2, 2)
...
mod:hook_origin(assault_new, "update_function", function(parent, ui_renderer, widget, template, crosshair_settings, dt, t, draw_hit_indicator)
	...
	if yaw and pitch then
		...
		for i=1,4 do
			styles[i].offset[1], styles[i].offset[2] = crosshairs_fix.crosshair_rotation(spread_offset_x, spread_offset_y, styles[i].angle, styles[i].size[1]/2, 2)
		end
	end
